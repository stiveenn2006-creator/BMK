...
const App: React.FC = () => {
  ...
  if (!isLoggedIn) {
    return (
      <div className={`min-h-screen flex flex-col bg-charcoal selection:bg-gold/30 selection:text-white overflow-x-hidden ${isAr ? 'font-arabic' : 'font-sans'}`} dir={isAr ? 'rtl' : 'ltr'}>
  ...
  return (
    <div className={`min-h-screen flex flex-col bg-charcoal selection:bg-gold/30 selection:text-white overflow-x-hidden pb-16 lg:pb-0 ${isAr ? 'font-arabic' : 'font-sans'}`} dir={isAr ? 'rtl' : 'ltr'}>
      ...
      {showProfile && <Profile onClose={() => setShowProfile(false)} lang={lang} setLang={setLang} onProductAdded={checkAndLoad} />}
